        ...
        <body>
            ...
            <table>
                <thead>
                    ...
                    <th>Markets</th><th>Selections</th><th>Last Snapshot</th><th>Page</th><th>Innings Tracker</th></tr>
                </thead>
                <tbody>{rows}</tbody>
            </table>
            <script>
...
def view_match_live_markets(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        flat_records = raw_results[0] if raw_results and isinstance(raw_results[0], list) else (raw_results if isinstance(raw_results, list) else [])
        ...
        for m in markets:
            ...
            rows += f"""
            <tr data-group="{group}">
                <td>{group}</td><td>{market_nm}</td><td>{selection}</td>
                <td>{escape(odds_display)}</td><td>{line}</td><td>{suspended}</td>
            </tr>
            """
        html = f"""
        <!DOCTYPE html>
        ...
        <body>
            ...
            <table>
                <thead><tr><th>Group</th><th>Market</th><th>Selection</th><th>Odds Decimal (Fractional)</th><th>Line</th><th>Suspended</th></tr></thead>
                <tbody>{rows}</tbody>
            </table>
            <script>
