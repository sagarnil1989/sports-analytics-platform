...
def view_ml_win_predictor_no_odds_config_post(req: func.HttpRequest) -> func.HttpResponse:
...
def view_ml_win_predictor_no_odds_html(req: func.HttpRequest) -> func.HttpResponse:
    try:
        return _view_ml_win_predictor_no_odds_html_inner(req)
...
def _view_ml_win_predictor_no_odds_html_inner(req: func.HttpRequest) -> func.HttpResponse:
    gold = get_named_container_client("gold")
    ...
    try:
        raw = gold.get_blob_client("cricket/ml_features/t20/win_predictor_no_odds_summary.json").download_blob().readall()
    ...
    if summary is None:
        body = "<p style='color:#c00'>No model results found. Run ml_win_predictor_no_odds in Databricks first.</p>"
    else:
        ...
        _odds_ctx = {}  # no odds features in this model
...
<body>
    ...
    function wiRun(panelId, model) {{
        ...
        fetch('/api/ml/win-predictor/whatif', {{
            method: 'POST',
            headers: {{'Content-Type': 'application/json'}},
            body: JSON.stringify({{ model: model, raw_features: rawFeatures, model_variant: 'no_odds' }})
        }})
