...
|---|---|---|
| Bronze capture (inplay + prematch) | Azure Function App — timer triggers | Every 5s–1hr, calls BetsAPI, writes raw snapshots |
| HTTP API routes | Azure Function App — HTTP triggers | On-demand reads from gold/bronze |
...
| Azure Functions Consumption | Bronze capture + HTTP routes | ~€0 at MVP scale |
